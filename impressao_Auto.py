...
pathLogoEnterprise = r'path\to\image'
paragraph1 = make_doc.add_paragraph()
paragraph1.add_run().add_picture(pathLogoEnterprise, width=Inches(1), height=Inches(0.8))
...
# Document Body
pathImageColor = r'path\to\image' # Path to image
...
# Save the Document
pathSaveDocument = r'path\to\save\document.docx' # Path to save document
make_doc.save(pathSaveDocument)



############################################## # PRINT ################################################ ##########
# Variable for path of log file
pathLog = 'path\\to\\save\\archive\\log.log'
# Start execution log
os.system(f"cmd /c echo. >>{pathLog}")
os.system(f"cmd /c echo ************ TEST ************ >>{pathLog}")
os.system(f"cmd /c echo. >>{pathLog}")
os.system(f"cmd /c echo --- Run on: {dt_e_hr_conv_cmd} >>{pathLog}")
...
os.system(f"cmd /c echo --- Received printer: {printer[2]} >>{pathLog}")

# Set the path of the print folder and the file to be printed
path = r"path\to\archive"
file = r"name\to\archive.docx"
os.system(f"cmd /c echo --- Printed Document: {file} >>{pathLog}")
...
# End the script
os.system(f"cmd /c echo. >>{pathLog}")
os.system(f"cmd /c echo --------------------End------------------- - >>{pathLog}")
